...
CaclaDriver::CaclaDriver()
{
	...
	g_reupdate_steps_per_tick = 1; // update verhouding = learn_steps : 9*reupdate_steps
...
const int CaclaDriver::gearUp[6]=
    {
        8000,9500,9500,9500,9500,0
    };
const int CaclaDriver::gearDown[6]=
    {
...
/* Stuck constants*/
const int CaclaDriver::stuckTime = 25;
const float CaclaDriver::stuckAngle = 0.785398163f; //.523598775f; //PI/6

/* Accel and Brake Constants*/
const float CaclaDriver::maxSpeedDist=70;
const float CaclaDriver::maxSpeed=150;
const float CaclaDriver::sin5 = 0.08716f;
const float CaclaDriver::cos5 = 0.99619f;

/* Steering constants*/
const float CaclaDriver::steerLock=0.785398f;
const float CaclaDriver::steerSensitivityOffset=80.0f;
const float CaclaDriver::wheelSensitivityCoeff=1;

/* ABS Filter Constants */
const float CaclaDriver::wheelRadius[4]={0.3179f,0.3179f,0.3276f,0.3276f};
const float CaclaDriver::absSlip=2.0f;
const float CaclaDriver::absRange=3.0f;
const float CaclaDriver::absMinSpeed=3.0f;

/* Clutch constants */
const float CaclaDriver::clutchMax=0.5f;
const float CaclaDriver::clutchDelta=0.05f;
const float CaclaDriver::clutchRange=0.82f;
const float CaclaDriver::clutchDeltaTime=0.02f;
const float CaclaDriver::clutchDeltaRaced=10;
const float CaclaDriver::clutchDec=0.01f;
const float CaclaDriver::clutchMaxModifier=1.3f;
const float CaclaDriver::clutchMaxTime=1.5f;

int CaclaDriver::getGear(CarState &cs)
{
...
float CaclaDriver::getSteer(CarState &cs)
{
...
float CaclaDriver::getAccel(CarState &cs)
{
...
void CaclaDriver::init(float *angles)
{
	...
	if (mp_Qinterface == NULL) {
		cout << "Creating CaclaLearningInterface...\n";
		try
		{
			initInterface(true);
			cout << "Done.\n";
...
{
	mp_Qinterface = new CaclaLearningI();
	ifstream is;
	ifstream is2;
	is.open("log_files/Cacla_ANN_9000.txt");
	is2.open("log_files/Cacla_VNN_9000.txt");
	if(load_network && is.is_open() && is2.is_open()) {
		is.close();
		is2.close();
		cout << "Loading NN from file.";
		mp_Qinterface->init("log_files/Cacla_ANN_9000.txt", "log_files/Cacla_VNN_9000.txt");
	}
	else
		mp_Qinterface->init();
}

CarControl CaclaDriver::wDrive(CarState cs)
{
	timeBeginPeriod(1);
	...
	if(g_count % g_steps_per_action == 0) 
	{
		...
		*gp_prev_state = cs;
		...
		DWORD end_learn = timeGetTime();
		DWORD  diff = end_learn - start_learn;

		debug_message << "normal learning - time taken: " << diff << endl;
		mp_log->write(debug_message.str());
	...
	} else
	{
			...
			//DE EERSTE g_steps_per_action STAPPEN VAN -->ELKE RONDE<-- UPDATE HIJ DUS NIET!!
		{
			...
			try{
				...
				while(g_reupdate_step_count < g_reupdate_steps_per_tick && !g_learning_done) {
					...
					mp_Qinterface->updateWithOldTuple(CaclaLearningI::TD);
		...
		DWORD  diff = end_learn - start_learn;
		stringstream debug_message;
		debug_message << "reupdate - time taken: " << diff << endl;
		mp_log->write(debug_message.str());
	}
	//END LEARNING CODE
	...
	stringstream debug_message;
	debug_message << "time taken: " << diff << endl;
	mp_log->write(debug_message.str());
	return rlControl(cs);
}

double CaclaDriver::computeReward(CarState &state, double* action, CarState &next_state)
{
...
void CaclaDriver::doLearning(CarState &cs) 
{
	if (g_count % (g_print_mod) == 0){
		//cout << "Time: " << g_count << ". ";
	...
	while(	l_learn_step_count < g_learn_steps_per_tick
		...
		if (l_learn_step_count == 0)
			g_learning_done = mp_Qinterface->learningUpdateStep(true, CaclaLearningI::TD); //false = do not store tuples
		else
			g_learning_done = mp_Qinterface->learningUpdateStep(false, CaclaLearningI::TD);
		l_learn_step_count++;
		g_learn_step_count++;
	}
...
CarControl CaclaDriver::carStuckControl(CarState & cs)
{
	debug_stuck_count++;
	float acc = 0.8f;
...
CarControl CaclaDriver::simpleBotControl(CarState &cs)
{
	// compute gear 
    int gear = getGear(cs);
...
CarControl CaclaDriver::rlControl(CarState &cs)
{
...
void CaclaDriver::endOfRunCheck(CarState &cs, CarControl &cc)
{
	...
	if(g_learn_step_count + g_reupdate_step_count >= 15000) //(g_learn_step_count >= 400) //or (g_count >= 4000) //(g_learn_step_count + g_reupdate_step_count >= 5000)
	{
		cc.setMeta(cc.META_RESTART);
		g_count = 0;
...
float CaclaDriver::filterABS(CarState &cs,float brake)
{
...
void CaclaDriver::clutching(CarState &cs, float &clutch)
{
...
char CaclaDriver::getKeyboardInput()
{
    if (_kbhit())
    {
